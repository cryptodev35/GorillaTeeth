...
// TEETH: optional setting to unlock wallet for block minting only;
...
// TEETH: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // TEETH: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // TEETH: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // TEETH: not avatar mode
                    {
...
// TEETH: create coin stake transaction
...
// TEETH: check 'spent' consistency between wallet and txindex
// TEETH: fix wallet spent state according to txindex
...
// TEETH: disable transaction (only for coinstake)
